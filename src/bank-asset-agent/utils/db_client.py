...
class QueueDatabaseClient:
    """Database client for queue-db"""
    
    def __init__(self, connection_string: str = None):
        self.connection_string = connection_string or os.environ.get('QUEUE_DB_URI')
        if not self.connection_string:
            raise ValueError("Queue database connection string not provided")
    ...
    def add_queue_entry(self, queue_data: Dict[str, Any]) -> str:
        """Add entry to investment queue"""
        try:
            with self.get_connection() as conn:
                cursor = conn.cursor()
                cursor.execute("""
                    INSERT INTO investment_queue (account_number, tier_1, tier_2, tier_3, uuid, status, created_at)
                    VALUES (%s, %s, %s, %s, %s, %s, CURRENT_TIMESTAMP)
                    RETURNING queue_id
                """, (
                    queue_data['account_number'],
                    queue_data['tier_1'],
                    queue_data['tier_2'],
                    queue_data['tier_3'],
                    queue_data['uuid'],
            ...
            raise Exception(f"Queue entry addition failed: {e}")
    
    def get_pending_requests(self, account_number: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                if account_number:
                    cursor.execute("""
                        SELECT queue_id, account_number, tier_1, tier_2, tier_3, uuid, status, created_at
    ...
    def update_request_status(self, queue_id: str, status: str) -> bool:
